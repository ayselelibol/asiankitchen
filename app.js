...
all.addEventListener("click", allmenulist);
korea.addEventListener("click", koreanmenu);
...
function allmenulist(){
    let allmenu=""
    let yeniarray= menu.map(function(menu){
       return allmenu += createMenu(menu)
    })
...
function koreanmenu(){
    let koreanmenu="";
    let koreanarray=menu.filter(function(menu) {
        if(menu.category=="Korea"){
        return (koreanmenu+= createMenu(menu))
        
    }})
    menuSection.innerHTML = koreanmenu;
}
...
function chinamenu(){
    ...
    menu.filter(function(menu){
        if(menu.category=="China"){
           return (chinamenu+= createMenu(menu));
        
    }})
    menuSection.innerHTML=chinamenu;
}
...
function japanmenu(){
    ...
    menu.filter(menu=>{
        if(menu.category=="Japan"){
          return (japanmenu += createMenu(menu));
        
    }})
    menuSection.innerHTML=japanmenu;
}

const createMenu = (food) => {

    let item = ` 
    <div style="padding-right:25px" class="menu-item row col-6 justify-content-between mb-5 mt-2">
